...
public class Ball {
    ...
    public void update(float elapsedTime, GameObject npcPaddle, Vector3f playerPosition) {
        float delta = elapsedTime / 1000f;
        float d = delta;
        while (d > 0f) {
            float step = Math.min(MAX_STEP, d);
            moveBall(step, npcPaddle, playerPosition);
            d -= step;
        }
    }

    private void moveBall(float delta, GameObject npcPaddle, Vector3f playerPosition) {
